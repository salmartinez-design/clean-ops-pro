...
async function main() {
  ...
  for (const c of clientDefs) {
    const existing = await db.select({ id: clientsTable.id }).from(clientsTable)
      .where(and(eq(clientsTable.company_id, COMPANY_ID), eq(clientsTable.email, c.email!))).limit(1);
    if (existing.length > 0) {
  ...
  // Map: clientIdx (0-based) -> clientId, employeeIdx (0-based) -> employeeId
  ...
  const jobDefs = [
    ...
    // 2 "flagged" — store as complete with flag note
    ...
    // 1 unassigned (future, no employee)
  ...
  for (const j of jobDefs) {
    const serviceMap: Record<string, string> = {
      recurring: "recurring", deep_clean: "deep_clean", office_cleaning: "office_cleaning",
    ...
    jobIds.push(row.id);
    console.log(`  Created job id=${row.id} client=${j.client} status=${j.status}`);
  }

  // ── 4. INVOICES ─────────────────────────────────────────────────────────────
  console.log("[demo-seed] Seeding invoices...");
  const invoiceDefs = [
    // 3 paid (for first 3 complete jobs)
    ...
    // 1 overdue (sent 14 days ago, due 10 days ago)
  ...
  todayBase.setHours(7, 30, 0, 0);

  const timeclockDefs = [
    // emp[2] (Sofia) — clocked in and out (job 5 = today in_progress Novak)
    { empIdx: 2, jobIdx: 5, inH: 7, inM: 30, outH: 11, outM: 15, flagged: false },
    // emp[3] (Darius) — clocked in and out (job 6 = today Sunrise)
    { empIdx: 3, jobIdx: 6, inH: 9, inM: 5,  outH: 12, outM: 45, flagged: false },
    // emp[7] (Kevin Osei) — currently clocked in, no clock out (job 7 = today Nina)
    { empIdx: 7, jobIdx: 7, inH: 10, inM: 35, outH: null, outM: null, flagged: false },
    // emp[0] (Maria) — flagged: clocked in 8 hours ago, no clock out
    { empIdx: 0, jobIdx: 0, inH: null, inM: null, outH: null, outM: null, flagged: true, hoursAgo: 8 },
    // emp[4] (Anika) — not clocked in today (no entry)
  ];
  ...
  for (const tc of timeclockDefs) {
    ...
    console.log(`  Timeclock: emp[${tc.empIdx}] job[${tc.jobIdx}] flagged=${tc.flagged}`);
  }

  // ── 6. SCORECARDS ────────────────────────────────────────────────────────────
  console.log("[demo-seed] Seeding scorecards...");
  const scoreDefs = [
    // Score 4 — from Mitchell job (job 0, client 0, emp 0)
    ...
    // Score 3 — from Kowalski job (job 1, client 1, emp 2)
    ...
    // Score 1 — from Hargrove job (job 2, client 5, emp 6)
  ...
  for (const s of scoreDefs) {
    ...
    console.log(`  Scorecard: job[${s.jobIdx}] score=${s.score}`);
  }

  // ── 7. CONTACT TICKET (auto from score=1 Hargrove) ─────────────────────────
  console.log("[demo-seed] Seeding contact ticket for low-score...");
  ...
  console.log("  Contact ticket created for Hargrove complaint.");

  console.log("\n[demo-seed] All done! Summary:");
  ...
  console.log(`  Timeclock: ${timeclockDefs.length}`);
  console.log(`  Scorecards: ${scoreDefs.length}`);
}
